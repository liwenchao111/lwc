...
class PlaceData(object):
    ...
    def pre_compute_var(self):
        ...
        die_area = torch.prod(self.die_ur - self.die_ll).item()
        mov_node_size = self.node_size[mov_lhs:mov_rhs, ...]
        mov_cell_area = torch.prod(mov_node_size, 1).sum().item()
        fixed_node_size = self.node_size[self.fixed_index[0]:self.fixed_index[1], ...]
        fixed_node_area = torch.prod(fixed_node_size, 1).sum().item()
        self.fixed_node_area_ratio = fixed_node_area / die_area
        self.mov_cell_area_ratio = mov_cell_area / die_area
        self.placeable_area_ratio = 1.0 - self.fixed_node_area_ratio
        whitespace_is_enough = (args.target_density - self.mov_cell_area_ratio / self.placeable_area_ratio) > 0.01
        self.use_whitespace_redistribution = whitespace_is_enough or ((self.mov_cell_area_ratio - self.fixed_node_area_ratio) > 0.7)
        return self
    ...
    def logging_statistics(self):
        ...
        mov_lhs, mov_rhs = self.movable_index
        top_10_node_to_num_pins_values, _ = self.node_to_num_pins[:,0].topk(10)
        top_10_mov_node_to_num_pins_values, _ = self.node_to_num_pins[mov_lhs:mov_rhs,0].topk(10)
        ...
        content += "Top 10 node to num pins = " + str([i for i in top_10_node_to_num_pins_values.cpu().numpy()]) + "\n"
        content += "Top 10 movable node to num pins = " + str([i for i in top_10_mov_node_to_num_pins_values.cpu().numpy()]) + "\n"
        content += (
        ...
        content += "target density = %.2f, fixed node area = %.2f, movable node area = %.2f\n" % (
            args.target_density,
            self.fixed_node_area_ratio,
            self.mov_cell_area_ratio,
        )        
        content += "==================="
        logger.info(content)
        return self
